...
## Pop-quiz, hot shot(s)...
![](https://y.yarn.co/4c18a944-8df2-4278-a69e-7d12fabb16e6_text.gif){width="1000" fig-align="center"}

## Pop-quiz, hot shot(s)...
![](https://i.ytimg.com/vi/lVTnOhJrMK4/maxresdefault.jpg)

## Pop-quiz, hot shot(s)...

Ok, but actually let's do a little mini-quiz since we're ~25% of the way though the semester! In groups, address the following questions:

. . . 

1. Imagine you've conducted a t-test (one, two sample, doesn't matter), and calculated a t-statistic of 3.6. Which t-distribution would yield a smaller p-value when calculated: $t_{15}$ or $t_{120}$? Why? 

. . . 

2. Related to number 1, what are the two things that we can change about an experiment/observational study, and in what direction should we change them, to *increase* the power of our statistical test? 

. . . 

3. What is the advantage of **standardizing** our data using a Z-distribution (i.e., Z-score)?

. . . 

4. Math time! Imagine we have a random variable, $Y \sim N(15, 2^2)$. Calculate the probability that our random variable takes the values between 15 and 19 (i.e., $P(15 ≤ Y ≤ 19)$). For the p-values, you can look it them with a simple Google search (in this case, the AI answer *should* be correct, last I checked)
...
:::box-6-list
Like any model, ANOVA has certain assumptions that must be met for our tests to be valid:
:::
...
2. Variance is homogeneous among groups (i.e., each group contributes ~ equally to the within group sum of squares)
